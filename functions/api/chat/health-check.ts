 ...
 */
async function healthCheckHandler(event: AuthenticatedRequest, context: HandlerContext) {
 ...
 */
export async function publicHealthCheck(event: any, context: HandlerContext) {
  // Ensure required environment variables are set
  if (!validateEnvironment()) {
    return createResponse({ success: false, message: 'Server configuration error' }, 500);
  }
...
export const handler = withAuth(healthCheckHandler); 